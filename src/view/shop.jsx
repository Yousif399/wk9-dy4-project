...
const ShopBikes = () => {
  ...
  const getBike = async () => {
    let response = await axios.get('https://bike-flask.onrender.com/api/bike');
...
//     copyCart.size ++;
//     copyCart.total += Math.floor(bike[0].id.data.price * 100) / 100
//     copyCart.bike[bike[0].id] ?
//       copyCart.bike[bike[0].id].quantity++
//       :
//       copyCart.bike[id] = { data: bike, quantity: 1 }
//     console.log('hello', copyCart);
//     if (user) {
...
//                   <Card.Text className="other-txt">${b.price}</Card.Text>
//                   <span className="shop-btn">
//                     {/* <Link className="add-btn" to='/cart' ><Button className="btn-outline" onClick={() => addBike(b.id)} >Buy</Button></Link> */}
//                     <Button className="add-btn" variant="outline-dark" onClick={() => addBike(b)} > Add to Cart
//                     </Button>
//                   </span>
//                 </Card.Body>
